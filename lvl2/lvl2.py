...
try:
    ...
    try:
        ...
        print("[+] Oh no, The filename has spaces in it!!")
        print("[+] But we can use double quotes or escape character to escape the spaces\n")

        print('[#] $ cat spaces\\ in\\ this\\ filename')
        cmd = s.process('cat spaces\\ in\\ this\\ filename',shell=True).recvall().decode(encoding)
        print("[#] $",cmd)
        print("[+] Wow this is the flag for next level bandit2")                

        with open('flag3.txt','w') as f:
            print("[+] Writting flag to flag3.txt")
            f.write(cmd.rstrip('\n'))
            print("[+] Done\n")
            sys.exit(1)
