...
try:
    ...
    try:
        ...
        print("[+] Hmm The readme file looks interesting..!!!\n")
        print("[#] $ cat readme")
        cmd = s.process(['cat','readme']).recvall().decode(encoding)
        print("[#] $",cmd)
        
        print("[+] Wow this is the flag for next level bandit1")
        

        with open('flag1.txt','w') as f:
            print("[+] Writting flag to flag1.txt")
            f.write(cmd.rstrip('\n'))
            print("[+] Done\n")
            sys.exit(1)
